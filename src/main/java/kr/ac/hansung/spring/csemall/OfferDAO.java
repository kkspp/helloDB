...
public class OfferDAO {
	...
	public void setDataSource(DataSource dataSource) {
		this.jdbcTemplateObject = new JdbcTemplate(dataSource);	//이 코드는 일반적으로 틀에 박힌 코드다.
	}
	/* DB에 쿼리 던지는 메소드*/
	public int getRowCount() {
		String sqlStatement = "select count(*) from offers";
		return jdbcTemplateObject.queryForObject(sqlStatement, Integer.class);//sql문 수행 후 하나의 오브젝트를 받아온다.
	}

	
	//한개의 레코드를 읽기. querying and returning a single object
	public Offer getOffer(String name) {
		String sqlStatement = "select * from offers where name=?";
		/*
		//new Object[]{name} 대신 name 써도 됨.
	...
	//모든 레코드를 읽기. querying and returning a multiple object
	...
	public boolean delete(int id) {
		...
		return (jdbcTemplateObject.update(sqlStatement, id)==1);//id 혹은 new Object[]{id}
	}
